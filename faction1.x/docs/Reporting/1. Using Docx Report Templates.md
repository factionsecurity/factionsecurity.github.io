...
- **${vulnTable Section_Name}** – This defines a table to be a vulnerability listing table for a section of vulnerabilities. See [Reporting Sections](/faction1.x/Reporting/Using%20Docx%20Report%20Templates/#reporting-sections-enterprisepaid-feature) (Paid Only Feature).
...
- **${color  key=value,key=value}** – The color of the text is based on key-value pairs. [See below for how to set up colors.](/faction1.x/Reporting/Using%20Docx%20Report%20Templates/#setting-severity-colors)
- **${cells key=value,key=value}** – The color of the table cell is based on key-value pairs.  [See below for how to set up colors.](/faction1.x/Reporting/Using%20Docx%20Report%20Templates/#setting-severity-colors)
...
- **${loop-*}** – This allows multiple rows to be repeated. For example, ${loop-1} will repeat the row and the one below it.
...
**Why is the heading yellow?!?!** Check [here](/faction1.x/Reporting/Using%20Docx%20Report%20Templates/#setting-severity-colors).
...
- **${fiBegin Section_Name} / ${fiEnd Section_Name}** – Block to repeat a section of findings. See [Reporting Sections](/faction1.x/Reporting/1.%20Using%20Docx%20Report%20Templates/#Reporting-Sections-(Enterprise/Paid-Feature)) (Paid Only Feature)
- **${vulnName}** – The Vulnerability name
- **${rec}** – Vulnerability Recommendation
- **${desc}** – Vulnerability Description
- **${category}** – Category of the vulnerability
...
- **${cfXXXXXX}** – Custom Fields are ones you specify in the admin interface. These are all prefixed with “cf”
- **${details}** – This will insert screenshots and exploit steps for each vulnerability.
- **${color  key=value,key=value}** – The color of the text is based on key-value pairs. [See below for how to set up colors.](/faction1.x/Reporting/1.%20Using%20Docx%20Report%20Templates/#setting-severity-colors)
- **${fill key=value,key=value}** – The color of the background elements is based on key-value pairs.  [See below for how to set up colors.](/faction1.x/Reporting/1.%20Using%20Docx%20Report%20Templates/#setting-severity-colors)
- **${custom-fields key,value,key=value,...}** - The color to match in the DOCX template. This allows you to set font colors for custom variables
...
**Why is the heading yellow?!?!** Check [here](/faction1.x/Reporting/1.%20Using%20Docx%20Report%20Templates/#setting-severity-colors).

Example Rendered:
![](/files/Pasted%20image%2020250728003841.png)

## Hyperlinks
If you want to add hyperlinks to variables, this can be done with some variables by appending `link` after the variable. There are only a few default variables that support this feature (`${asmtAssessor_Email link}`, `${cvssString link}`) but every Custom Variable will support this. This allows you to add things like `${cfAffectedURL link}` to your Vulnerability templates which will generate a hyperlink to whatever URL you enter. 
...
Right-click the overall severity variable, **${severity}**; you can see the default hex code for this color is #FAC701. Likelihood would be set to #FAC702, and Impact would be set to #FAC703.
...
We can now set the color based on the text that gets entered into these fields. Let's say if the text displays "Happy", then the background will be green, and if it says "Sad", then the background will be red. 
