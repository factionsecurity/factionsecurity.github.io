...
- Including your own application inventory database in the application search features and populating results in Faction, like the Application ID, Distribution Lists, and Application Name. 
...
- Triggers when a vulnerability state is changed. When a tracked vulnerability is retested and passes/fails, you can create a custom workflow and alert key stakeholders that the issue succeeded or failed. 
...
The full documentation on the API can be found [here](/faction1.x/APIS/App%20Store%20Extension%20API).
...
You can use this to create one extension that returns a JIRA Tracking number for all vulnerabilities in the finalized assessment, then take those numbers and process them into another system and on and on down the chain. 


### Example Extensions
You can review the source code for our initial apps [here](/faction1.x/Faction%20App%20Store%20Extensions). This list will grow over time but currently, there are 2 (JIRA Integration, Vulnerability Bar Charts). 
