...
date: 2023-12-18
---
If you decide to self-host Faction instead of using the [Managed Solution](/faction1.x/Managed%20FACTION%20Setup/) then you will need to ensure you include the proper environment variables so that Faction integrates into your environment.
...
- **FACTION_MONGO_HOST** (_Required_):  
    This is the hostname or IP address where your Mongo database is located.
...
- **FACTION_MONGO_AUTH_DATABASE** (_Optional_):  
...
- **FACTION_REPORT_STORAGE** (_Optional_):  
    This variable has two options: **local** or **aws**. If you include **aws** then you also need to set **AWS_ACCESS_KEY_ID** and **AWS_SECRET_ACCESS_KEY** and create an S3 bucket location in AWS. _You can also use IAM permissions instead of including the AWS Keys._   
    When using **local**, the default directory location is **/opt/faction**. If this variable is not set, **local** will be used as the default.
- **FACTION_BUCKET_NAME** (_Optional_):  
    S3 bucket name to store Faction files.
