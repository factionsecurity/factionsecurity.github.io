...
```
3. Create the following folders and files. They can be empty for now. We will fill these out in the next section below.
...
- `resources/META-INF/resources/logo.png`: This is a PNG logo that will be displayed as the app's icon in the Faction App Store UI.
- `resources/META-INF/services/*`: This file is required for Faction to be able to load your extension.

### config.json
These are the key-value pairs that will display in the Faction App Store UI and allow users to make changes that affect the behavior of the extensions. For a Jira plugin, you would want the API key, the Jira host, and the Jira username configured. That would look like the following:
```json
{
	"Jira Host" : {
		"type": "text",
...
Any values you set in your extension will be the "Default" values that will display to the end user. They can be blank if you do not want to set a predefined value.
...
### description.md
This is where you describe your extension to end users, and it supports most Markdown elements. You should use this to fully describe what the extension does, why the user should install it, and how the end user should configure it to work correctly.
...
Now that the basic setup is complete, we can create our Jira class that actually does something!
...
Note that above it returns **AssessmentManagerResult.** This object will update Faction’s database if the values change when this function returns. If you return **null**, it will NOT update Faction and will only send/process information.

## Send the Issues to Jira on Assessment Finalized
Now that we have a basic functional block of code, we need to make it perform the action of sending all vulnerabilities to Jira only when the assessment is finalized. To do this, we create an if statement that checks that the Operation enum equals Finalized and then sends the issues with our custom function `sendVulnerabilityToJira()`. We then get the tracking ID from Jira and update Faction so the tracking IDs are in sync.
